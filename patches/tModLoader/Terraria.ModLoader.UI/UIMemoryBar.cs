...
namespace Terraria.ModLoader.UI
{
	internal class UIMemoryBar : UIElement
	{
		...
		private void RecalculateMemory() {
			...
			long remainingMemory = _maxMemory - allocatedMemory;
			Console.WriteLine(remainingMemory);
			_memoryBarItems.Add(new MemoryBarItem(
				$"Available Memory: {SizeSuffix(remainingMemory)}\n Total: {SizeSuffix(availableMemory)}",
				remainingMemory, Color.Gray));

			//portion = (_maxMemory - availableMemory - meminuse) / (float)_maxMemory;
			//_memoryBarItems.Add(new MemoryBarItem($"Other programs: {SizeSuffix(_maxMemory - availableMemory - meminuse)}", portion, Color.Black));
