	...
	class Core64
	{
		...
		internal static void DrawPatreon(SpriteBatch sb, int num109, int num110, int num111, bool hasFocus, Color color12)
		{
			...
			if(showPatreon)
			{
				...
				Main.spriteBatch.DrawString(Main.fontMouseText, patreonShortURL, new Vector2((float)num110 + 10f, (float)Main.screenHeight - origin3.Y - 50f - (float)num111 + 2f), color12, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
				...
				if(!vanillaMode)
				{
					...
					Main.spriteBatch.DrawString(Main.fontMouseText, tmlModeString, new Vector2((float)num110 + 10f, (float)Main.screenHeight - origin3.Y - 72f - (float)num111 + 2f), color12, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
				...
				if(chest != null)
				{
					...
					for(int j = chest.x; j <= chest.x + 1; j++)
					{
						for(int k = chest.y; k <= chest.y + 1; k++)
						{
							...
							if(!tile.active() || (!Main.tileContainer[(int)tile.type] && tile.type != ModContent.TileType<MysteryTile>()))
							{
								flag = true;
								break;
							}
		...
		{
			int numberOfChest = (int)reader.ReadInt16();
